...
async def main():
    ...
    llm_client = GeminiClient(
        config=LLMConfig(
            ...
            model="gemini-2.0-flash",  # Main model for large tasks
            small_model="gemini-2.0-flash"  # Override broken default for small tasks
        )
    )
    ...
    embedder = GeminiEmbedder(
        config=GeminiEmbedderConfig(
            api_key=google_api_key,
            embedding_model="gemini-embedding-001"  # Correct Gemini embedding model
        )
    )
    ...
    cross_encoder = GeminiRerankerClient(
        config=LLMConfig(
            api_key=google_api_key,
            model="gemini-2.0-flash"  # Using same model as LLM for reranking
        )
    )
    
    # Initialize Graphiti with all Gemini components
    graphiti = Graphiti(
    ...
    try:
        ...
        # NOTE: Processing all 4 episodes with delays to respect free tier rate limits
        # Each episode uses ~5-15 LLM calls depending on complexity
        episodes = [
            {
        ...
        for i, episode in enumerate(episodes):
            ...
            # Wait 20 seconds between episodes to avoid rate limits (15 req/min for gemini-2.0-flash)
            if i < len(episodes) - 1:  # Don't wait after the last episode
                print(f'Waiting 20 seconds before processing next episode to respect rate limits...')
